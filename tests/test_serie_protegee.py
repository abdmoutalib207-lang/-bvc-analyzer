...
⚠️ ELLE FIGE LE TITRE, ET C'EST VOULU. Tant que le fichier est dans
`datasets/series_acceptees/`, aucune séance nouvelle n'entre par l'import. Pour
un titre suspendu c'est exactement la bonne règle ; le jour où il reprend, il
faut retirer le fichier — un acte délibéré, pas un effet de bord.
"""
...
def test_la_serie_publiee_de_cmt_est_celle_qui_a_ete_acceptee():
    ...
    publiee = json.loads((RACINE / "pipeline" / "candles" / "CMT.json")
                         .read_text(encoding="utf-8"))
    assert len(publiee) == 681 == acceptee["seances_negociees"]
    assert publiee[-1]["d"] == "2026-07-16" and publiee[-1]["c"] == 4350.0
