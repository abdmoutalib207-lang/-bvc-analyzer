...
AVANT_LA_PORTE = {"vérifier session bvc"}
...
def test_la_porte_existe_et_porte_l_identifiant_attendu(etapes):
    ...
    porte = [e for e in etapes if _nom(e) in AVANT_LA_PORTE]
    assert len(porte) == 1, "la porte de session doit exister, une seule fois"
    assert porte[0].get("id") == "gate"


def test_le_checkout_est_bien_conditionne(etapes):
    """C'est lui qui pose le dépôt. S'il n'est pas conditionné, la porte ne
    sert à rien ; s'il l'est, tout ce qui suit doit l'être aussi."""
    checkout = [e for e in etapes if _nom(e) == "checkout"]
    assert len(checkout) == 1
    assert "steps.gate.outputs.run" in str(checkout[0].get("if", ""))
...
def test_aucune_etape_ne_tourne_sans_depot(etapes):
    ...
    assert not fautives, (
        "ces étapes s'exécuteraient sur un runner sans dépôt : "
        + ", ".join(fautives))
